package com.nightfeed.wendu.net

import com.baidu.location.BDAbstractLocationListener
import com.baidu.location.BDLocation
import com.baidu.location.LocationClient
import com.baidu.location.LocationClientOption
import com.nightfeed.wendu.MyApplication
import com.nightfeed.wendu.utils.ToastUtil


public class WLocationClient private constructor() {

    interface OnResultListener{
        fun onSuccess(location: BDLocation)
        fun onNetError()
        fun onPermissionError()
    }
    companion object {
        private var onResultListener: OnResultListener?=null

        lateinit var mLocationClient :LocationClient
        lateinit var myListener : BDAbstractLocationListener

       private var option : LocationClientOption? =null

        /*单例*/
        @Volatile
        private var instance: WLocationClient? = null
            get() {
                if (field == null) {
                    field = WLocationClient()

                }
                return field
            }

        fun get(): WLocationClient{
            mLocationClient = LocationClient(MyApplication.context)

            myListener =object : BDAbstractLocationListener() {
                override fun onReceiveLocation(location: BDLocation) {
                    //此处的BDLocation为定位结果信息类，通过它的各种get方法可获取定位相关的全部结果
                    //以下只列举部分获取经纬度相关（常用）的结果信息
                    //更多结果信息获取说明，请参照类参考中BDLocation类中的说明

                    val resultCode =  location.getLocType()
                    if(resultCode==61||resultCode==66||resultCode==161){
                        onResultListener?.onSuccess(location)

                        val latitude = location.getLatitude()    //获取纬度信息
                        val longitude = location.getLongitude()    //获取经度信息
                        val radius = location.getRadius()    //获取定位精度，默认值为0.0f

                        val addr = location.addrStr    //获取详细地址信息
                        val country = location.country    //获取国家
                        val province = location.province    //获取省份
                        val city = location.city    //获取城市
                        val district = location.district    //获取区县
                        val street = location.street    //获取街道信息

                        val locationDescribe = location.locationDescribe    //获取位置描述信息

                        val coorType = location.getCoorType()
                        //获取经纬度坐标类型，以LocationClientOption中设置过的坐标类型为准

                    }else if(resultCode==167){
                        onResultListener?.onPermissionError()
                        ToastUtil.showError(MyApplication.context, "定位权限异常啦，请检查手机定位设置")
                    }else{
                        onResultListener?.onNetError()
                        ToastUtil.showNetError(MyApplication.context)
                    }

                }
            }

            mLocationClient.registerLocationListener(myListener)

            return instance!!
        }
    }

    fun startLocationClient(){
        if(option ==null){
            option =LocationClientOption()

            option!!.setLocationMode(LocationClientOption.LocationMode.Hight_Accuracy)
            //可选，设置定位模式，默认高精度
            //LocationMode.Hight_Accuracy：高精度；
            //LocationMode. Battery_Saving：低功耗；
            //LocationMode. Device_Sensors：仅使用设备；

            option!!.setCoorType("bd09ll")
            //可选，设置返回经纬度坐标类型，默认GCJ02 GCJ02：国测局坐标；BD09ll：百度经纬度坐标；BD09：百度墨卡托坐标；
            //海外地区定位，无需设置坐标类型，统一返回WGS84类型坐标

            option!!.setScanSpan(0)
            //可选，设置发起定位请求的间隔，int类型，单位ms
            //如果设置为0，则代表单次定位，即仅定位一次，默认为0
            //如果设置非0，需设置1000ms以上才有效

            option!!.setOpenGps(true)
            //可选，设置是否使用gps，默认false
            //使用高精度和仅用设备两种定位模式的，参数必须设置为true

            option!!.setLocationNotify(true)
            //可选，设置是否当GPS有效时按照1S/1次频率输出GPS结果，默认false

            option!!.setIgnoreKillProcess(true)
            //可选，定位SDK内部是一个service，并放到了独立进程。
            //设置是否在stop的时候杀死这个进程，默认（建议）不杀死，即setIgnoreKillProcess(true)

            option!!.SetIgnoreCacheException(true)
            //可选，设置是否收集Crash信息，默认收集，即参数为false

            option!!.setIsNeedAddress(true) //可选，是否需要地址信息，默认为不需要，即参数为false

            mLocationClient.setLocOption(option)
            //mLocationClient为第二步初始化过的LocationClient对象
            //需将配置好的LocationClientOption对象，通过setLocOption方法传递给LocationClient对象使用
            //更多LocationClientOption的配置，请参照类参考中LocationClientOption类的详细说明

        }
        mLocationClient.start()
    }

    fun stopLocationClient(){
        mLocationClient.stop()
    }

    fun setResultListener(onResultListener: OnResultListener){
        Companion.onResultListener =onResultListener
    }
}